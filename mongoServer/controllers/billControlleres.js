...
const updateStatusOfTheBill = async (req, res, next) => {
  const { id, hospitalId } = req.params
  console.log(id)
  try {
    const Bill = await GeneralBill.findOne({
      billId: id,
    ...
    if (Bill) {
      ...
      await Bill.save()
      return res.status(200).json({ success: true, message: "Bill status updated successfully!" });
    }
    else {
    ...
    console.log(e);
    return res.status(500).json({ success: false, message: "Bill status update failed!" });
  }
};
